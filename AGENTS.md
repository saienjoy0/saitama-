...
AIは監修済み候補ID・手順IDの対応付けと、親向け下書きに限定する。候補の表示文は監修済み文章のみ。新規生成文章は親確認前に子へ直接表示しない。モデルに公開・通知・報酬・権限の変更手段を渡さない。親が承認した内容版と宛先を公開時に再検証する。子どもの意思、保護者の承認、AI処理同意を分ける。

## 資料の優先関係
最新のユーザー方針 > 状態ファイルが指すレビュー用設計（承認済みとは限らない） > 旧資料の根拠・仮説。旧資料の「唯一の正本」「自動評価」は現在の承認や実装要件を意味しない。三者・循環とAIの詳細はv0.4追補、F18の課題分解はai-engine-review-v0.4.1を優先する。v0.3の共通UIとv0.2の下書き契約は追補が変更していない範囲で使用し、v0.1のAI設計は履歴。矛盾は `docs/design/DECISIONS.md` に記録する。
